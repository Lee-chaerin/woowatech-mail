...
const Home = () => {
        ...
        if (data && data.exists === true) {
          ...
          sendVerifyCodeMutation.mutate(email, {
            ...
            onError: (error) => {
              ...
              setMessage(
                serverMessage || "메일 발송 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요.",
              );
              setIsError(true);
              setIsCodeInputVisible(false);
            },
  ...
  const handleCheckVerifyCode = () => {
    ...
    checkVerifyCodeMutation.mutate(
      { email, code, category_id: category },
      {
        onSuccess: () => {
          setMessage("인증되었습니다.");
          setIsError(false);
      ...
      <div className="flex flex-col gap-2 md:gap-4.5 text-4xl md:text-7xl font-bold text-center text-white z-10">
        ...
        <div className="mt-6 flex flex-col gap-2.5 md:gap-3">
          <Select onValueChange={setCategory}>
            <SelectTrigger
              className="font-hannaAir font-semibold text-base md:text-xl bg-white/75 text-[#333333] h-10 md:h-11"
          ...
          <div className="flex flex-row gap-2.5">
            <Input
              ...
              className="font-hannaAir font-semibold bg-white/75 text-[#333333] h-10 md:h-11"
            ...
            <Button
              type="button"
              variant="outline"
              className="font-hannaAir font-semibold text-base md:text-xl text-black hover:bg-[#ffffffe0] hover:[#333333] h-10 md:h-11"
          ...
          {message && (
            <p
              className={`font-hannaAir font-semibold text-sm md:text-base ${isError ? "text-red-400" : "text-green-400"} text-left`}
            >
              {message}
            </p>
          )}
